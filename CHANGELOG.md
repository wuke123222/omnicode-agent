...
- 新增统一设计 token 层（间距/圆角/控件高度/自适应卡片阴影），深浅主题下卡片层级更清晰。
- 主按钮、发送按钮和悬停/按压反馈改为按填充色亮度自动选择可读前景与反馈方向，自定义强调色和工作坊皮肤下不再丢失对比度。
- 侧边栏导航按压态跟随工作坊皮肤；建议卡片增加静置描边；实验与科研面板改用圆角卡片并与整体视觉统一。
- 输入区快捷键提示按平台显示 ⌘↵ / Ctrl↵；修复助手回复"已复制"反馈计时器在消息移除后仍触发的问题。
