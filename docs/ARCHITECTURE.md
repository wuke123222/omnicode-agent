...
OpenCode 任务使用独立的 `omnicode-agent.db` 会话数据库，并把子进程的 XDG cache/state 指向 JetBrains system 目录下的 OmniCode 专用运行区，避免与 JetBrains 内置 ACP 或用户终端的不同 OpenCode 版本共享数据库、模型目录和维护锁；XDG data/config 不变，因此认证、供应商配置和工具仍沿用用户配置。它的有界 stderr 只映射为本地初始化、会话创建、项目快照、模型连接和上游重试等稳定阶段，不显示或持久化原文；30 秒仍未创建本地会话时终止子进程，该边界不覆盖已经开始的模型排队或生成。
