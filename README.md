...
- 独立 **Semi Design 图转码** 工作流：可直接选择或复用已上传的 UI 截图，先有界预检 monorepo 内的 React 版本、前端包、Semi 依赖和包管理器，再配置页面/组件、TSX/JSX、目标路径、样式、响应式和可访问性并进入 Agent；React 19 自动选择 `@douyinfe/semi-ui-19`，React 16–18 使用 `@douyinfe/semi-ui`。`!semi-design` 仍作为快速提示词保留
...
输入框右上角的 **图转码** 是独立工作流，不是只插入一段提示词。没有图片时它先打开图片选择器；已有图片时在后台以 6 层目录、5000 个目录和 16 个 `package.json` 为上限做只读预检，并跳过符号链接、依赖和构建目录。确认面板会展示前端包、React/Semi 兼容包、包管理器、主输出文件及执行边界。开始后只消费本次图片，其他附件草稿保留；视觉分析使用主视觉模型或已配置的视觉辅助模型，文件修改、可选依赖清单更新和验证命令仍分别经过现有审批、沙箱、任务审阅和回退。插件不会静默执行 `npm/pnpm/yarn install`，也不会把图片二进制写入 checkpoint。
