...
9. 在聊天底栏选择 **思考** 档位。全速会使用当前模型可验证的最高推理能力，并同步增加单轮输出余量与请求超时；长任务的循环和时间保护可在 **运行控制** 调整。
...
上下文受限时，选择器会保护当前运行的用户目标、验收条件和约束，而不会把纯工具结果误当成新的用户请求。触发循环/时间等运行保护、取消或 Agent 内部失败会生成有界、确定性的“已完成 / 证据 / 剩余 / 风险”部分结果，不额外消耗模型或工具调用；常见认证、权限、限流、网络、模型能力、运行边界、沙箱和配置错误会引导到对应修复入口。图片及其他附件二进制不写入 checkpoint；已经提取为消息文本的 Markdown、PDF 或其他文本内容可能作为脱敏、有界文本快照保留。
